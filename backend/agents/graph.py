...
from agents.tools import web_search, get_macro_indicator, get_company_info, get_price_trend
...
def web_scout_node(state: AgentState) -> dict:
    """Search for recent news and analyst coverage for the symbol."""
    symbol = state["symbol"]
    try:
        news = web_search.invoke(f"{symbol} stock news India 2024 outlook analyst")
        earnings = web_search.invoke(f"{symbol} quarterly results earnings recent")
        sources = []
        for line in (news + earnings).split("\n"):
        ...
        llm = _get_llm()
        resp = llm.invoke([
            SystemMessage(content="You are a financial analyst. Summarize news sentiment in 3 concise bullet points. Be factual, not promotional. End with: SENTIMENT_SCORE: <number 0-100>"),
...
def macro_lens_node(state: AgentState) -> dict:
    """Assess macro environment and its impact on this symbol's sector."""
    symbol = state["symbol"]
    sector = state["sector"]
    try:
        ...
        india_macro = web_search.invoke(f"India {sector} sector macro outlook RBI policy 2024")

        llm = _get_llm()
        resp = llm.invoke([
            SystemMessage(content=
                "You are a macro analyst. Given indicators and news, assess impact on the specified sector. "
                "Write 2-3 bullet points. End with: MACRO_SCORE: <number 0-100> (100=very bullish macro for this sector)"
            ),
